...
import com.shop.user.domain.ProductSubCategory;
...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("SELECT p FROM Product p " +
			"WHERE (:category = '' OR " +
			"      LOWER(p.category.name) = LOWER(:category) OR " +
			"      LOWER(p.category.parentCategory.name) = LOWER(:category) OR " +
			"      LOWER(p.category.parentCategory.parentCategory.name) = LOWER(:category)) " +
